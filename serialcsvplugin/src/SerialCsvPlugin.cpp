...
<OxygenPlugin name="SERIAL_CSV_PLUGIN" version="1.3" uuid="674c23a5-590a-4c7f-8aac-a97ed94a0ea5">
...
public:
    ...
    bool setup(const std::vector<odk::Property>& properties) override
    {
        ...
        m_num_channels = std::stoul(numchannels);
        ...
        if (m_num_channels > 0)
        {
...

            {
                ...
                name_vec = odk::split(channel_names, ',');
...

            {
                ...
                mins_vec = odk::split(channel_mins, ',');
...

            {
                ...
                maxs_vec = odk::split(channel_maxs, ',');
...

            {
                ...
                units_vec = odk::split(channel_units, ',');
    ...
    void prepareProcessing(odk::IfHost *host) override
    {
        ...
        baudrate = std::stoul(m_baudrate_prop->getValue());
    ...
    std::uint64_t selectComPort(const odk::PropertyList& params, odk::PropertyList& returns)
    {
        ...
        if (!serialport.empty())
        {
            try
            {
                ...
                uint32_t baudrate = std::stoul(baudrate_string);
